function [layout, cfg] = bst_artifactscantool_prepare_layout(cfg, data)
...
%
% Use as
%   layout = bst_artifactscantool_prepare_layout(cfg, data)
%
...
% these are used by the artifactscantool_preamble/artifactscantool_postamble function and scripts
bst_artifactscantool_revision = '$Id$';
bst_artifactscantool_nargin   = nargin;
bst_artifactscantool_nargout  = nargout;

% do the general setup of the function
% artifactscantool_defaults
% % artifactscantool_preamble init
% artifactscantool_preamble loadvar    data
%artifactscantool_preamble provenance data

% the artifactscantool_abort variable is set to true or false in artifactscantool_preamble_init
% if artifactscantool_abort
%   return
% end
...
if ~hasdata
  ...
  data = bst_artifactscantool_checkdata(data);
end

% check if the input cfg is valid for this function
%cfg = artifactscantool_checkconfig(cfg, 'renamed', {'elecfile', 'elec'});
%cfg = artifactscantool_checkconfig(cfg, 'renamed', {'gradfile', 'grad'});
%cfg = artifactscantool_checkconfig(cfg, 'renamed', {'optofile', 'opto'});

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% set default configuration options
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

cfg.rotate       = bst_ast_getopt(cfg, 'rotate',     []); % [] => default rotation is determined based on the type of sensors
cfg.center       = bst_ast_getopt(cfg, 'center',     'no');
cfg.style        = bst_ast_getopt(cfg, 'style',      '2d');
cfg.projection   = bst_ast_getopt(cfg, 'projection', 'polar');
cfg.layout       = bst_ast_getopt(cfg, 'layout',     []);
cfg.grad         = bst_ast_getopt(cfg, 'grad',       []);
cfg.elec         = bst_ast_getopt(cfg, 'elec',       []);
cfg.opto         = bst_ast_getopt(cfg, 'opto',       []);
cfg.output       = bst_ast_getopt(cfg, 'output',     []);
cfg.feedback     = bst_ast_getopt(cfg, 'feedback',   'no');
cfg.montage      = bst_ast_getopt(cfg, 'montage',    'no');
cfg.image        = bst_ast_getopt(cfg, 'image',      []);
cfg.mesh         = bst_ast_getopt(cfg, 'mesh',       []); % experimental, should only work with meshes defined in 2D
cfg.bw           = bst_ast_getopt(cfg, 'bw',         'no');
cfg.channel      = bst_ast_getopt(cfg, 'channel',    'all');
cfg.skipscale    = bst_ast_getopt(cfg, 'skipscale',  []); % see below
cfg.skipcomnt    = bst_ast_getopt(cfg, 'skipcomnt',  []); % see below
cfg.boxchannel   = bst_ast_getopt(cfg, 'boxchannel', cfg.channel);
cfg.overlap      = bst_ast_getopt(cfg, 'overlap',    'shift');
cfg.viewpoint    = bst_ast_getopt(cfg, 'viewpoint',  []);
cfg.headshape    = bst_ast_getopt(cfg, 'headshape',  []); % separate form cfg.mesh
cfg.mri          = bst_ast_getopt(cfg, 'mri',        []);
cfg.outline      = bst_ast_getopt(cfg, 'outline',    []); % default is handled below
cfg.mask         = bst_ast_getopt(cfg, 'mask',       []); % default is handled below
cfg.width        = bst_ast_getopt(cfg, 'width',      []);
cfg.height       = bst_ast_getopt(cfg, 'height',     []);
cfg.commentpos   = bst_ast_getopt(cfg, 'commentpos', 'layout');
cfg.scalepos     = bst_ast_getopt(cfg, 'scalepos',   'layout');
...
if ~isempty(cfg.headshape) && ~isempty(cfg.mri)
  bst_artifactscantool_error('cfg.headshape and cfg.mri are mutually exclusive, please use only one of the two')
end
...
if ~isempty(cfg.viewpoint) && ~isequal(cfg.projection, 'orthographic')
  bst_artifactscantool_error('cfg.viewpoint can only used in the case of orthographic projection')
end
if ~isempty(cfg.viewpoint) && ~isempty(cfg.rotate)
  bst_artifactscantool_error('cfg.viewpoint and cfg.rotate are mutually exclusive, please use only one of the two')
end

% update the selection of channels according to the data
if hasdata && isfield(data, 'topolabel')
  cfg.channel = bst_ast_channelselection(cfg.channel, data.topolabel);
elseif hasdata && isfield(data, 'label')
  cfg.channel = bst_ast_channelselection(cfg.channel, data.label);
elseif hasdata && isfield(data, 'labelcmb')
  cfg.channel = bst_ast_channelselection(cfg.channel, unique(data.labelcmb(:)));
end

if ischar(cfg.layout) && strcmp(cfg.layout, 'vertical')
  cfg.direction = bst_ast_getopt(cfg, 'direction', 'TB'); % default is top-to-bottom
elseif ischar(cfg.layout) && strcmp(cfg.layout, 'horizontal')
  cfg.direction = bst_ast_getopt(cfg, 'direction', 'LR'); % default is left-to-right
elseif ischar(cfg.layout) && strcmp(cfg.layout, 'ordered')
  cfg.direction = bst_ast_getopt(cfg, 'direction', 'LRTB'); % default is left-to-right, then top-to-bottom
end
...
skipscale = bst_ast_istrue(cfg.skipscale); % in general a scale is desired
skipcomnt = bst_ast_istrue(cfg.skipcomnt); % in general a comment desired
...
elseif isequal(cfg.layout, 'circular')
  ...
  if isempty(rho)
    ...
    if numel(rho) ~= nchan
      bst_artifactscantool_error('the number of elements in the polar angle vector should be equal to the number of channels');
    end
...
elseif isequal(cfg.layout, 'vertical') || isequal(cfg.layout, 'horizontal')
  if hasdata && ~isempty(data)
    ...
    if iscell(originalorder) && length(originalorder)==length(cfg.channel)
      ...
      [~, sel] = bst_ast_match_str(originalorder, cfg.channel);
  ...
  switch cfg.layout
    case 'vertical'
      cfg.width  = bst_ast_getopt(cfg, 'width',  0.8);
      cfg.height = bst_ast_getopt(cfg, 'height', 0.8 * 1/(nchan+1+2));
    case 'horizontal'
      cfg.width  = bst_ast_getopt(cfg, 'width',  0.8 * 1/(nchan+1+2));
      cfg.height = bst_ast_getopt(cfg, 'height', 0.8);
  end
    ...
    switch cfg.layout
      case 'vertical'
        switch upper(cfg.direction)
          ...
          otherwise
            bst_artifactscantool_error('invalid direction "%s" for "%s"', cfg.direction, cfg.layout);
        end
        x = 0.5*(cfg.width/0.8);
        layout.pos   (i,:) = [x y];
...
elseif isequal(cfg.layout, 'ordered')
  ...
  cfg.width  = bst_ast_getopt(cfg, 'width',  0.8 * 1/ncol);
  cfg.height = bst_ast_getopt(cfg, 'height', 0.8 * 1/nrow);
...
elseif ischar(cfg.layout)
  sprintf('reading layout from file %s\n', cfg.layout);
  layout = readlay(cfg.layout);
  
elseif ~isempty(cfg.grad)
  if isstruct(cfg.grad)
    bst_artifactscantool_info('creating layout from cfg.grad\n');
    sens = bst_artifactscantool_datatype_sens(cfg.grad);
  else
    sprintf('creating layout from gradiometer file %s\n', cfg.grad);
    sens = artifactscantool_read_sens(cfg.grad, 'senstype', 'meg');
  end
...
elseif isfield(data, 'grad') && isstruct(data.grad)
  bst_artifactscantool_info('creating layout from data.grad\n');
  sens = bst_artifactscantool_datatype_sens(data.grad);
  layout = sens2lay(sens, cfg.rotate, cfg.projection, cfg.style, cfg.overlap, cfg.viewpoint, cfg.boxchannel);

elseif ~isempty(cfg.elec)
  if isstruct(cfg.elec)
    bst_artifactscantool_info('creating layout from cfg.elec\n');
    sens = bst_artifactscantool_datatype_sens(cfg.elec);
  else
    bst_artifactscantool_info('creating layout from electrode file %s\n', cfg.elec);
    sens = artifactscantool_read_sens(cfg.elec, 'senstype', 'eeg');
  end
  layout = sens2lay(sens, cfg.rotate, cfg.projection, cfg.style, cfg.overlap, cfg.viewpoint, cfg.boxchannel);

elseif isfield(data, 'elec') && isstruct(data.elec)
  bst_artifactscantool_info('creating layout from data.elec\n');
  sens = bst_artifactscantool_datatype_sens(data.elec);
  layout = sens2lay(sens, cfg.rotate, cfg.projection, cfg.style, cfg.overlap, cfg.viewpoint, cfg.boxchannel);

elseif ~isempty(cfg.opto)
  if isstruct(cfg.opto)
    bst_artifactscantool_info('creating layout from cfg.opto\n');
    sens = bst_artifactscantool_datatype_sens(cfg.opto);
  else
    bst_artifactscantool_info('creating layout from optode file %s\n', cfg.opto);
    sens = artifactscantool_read_sens(cfg.opto, 'senstype', 'nirs');
  end
  if (hasdata)
...
elseif isfield(data, 'opto') && isstruct(data.opto)
  bst_artifactscantool_info('creating layout from data.opto\n');
  sens = bst_artifactscantool_datatype_sens(data.opto);
  if (hasdata)
    layout = opto2lay(sens, data.label, cfg.rotate);
  else
    layout = opto2lay(sens, sens.label, cfg.rotate);
...
elseif (~isempty(cfg.image) || ~isempty(cfg.mesh)) && isempty(cfg.layout)
  if ~isempty(cfg.image)
    ...
    bst_artifactscantool_info('reading background image from %s\n', cfg.image);
  ...
  elseif ~isempty(cfg.mesh)
    if isfield(cfg.mesh, 'sulc')
      bst_artifactscantool_plot_mesh(cfg.mesh, 'edgecolor', 'none', 'vertexcolor', cfg.mesh.sulc); colormap gray;
    else
      bst_artifactscantool_plot_mesh(cfg.mesh, 'edgecolor', 'none');
    end
  end
  hold on
  axis equal
  ...
  while again
    ...
    switch k
      ...
      case 8
        if size(pos,1)>0
          ...
          if ~isempty(cfg.image)
            ...
            h = bst_artifactscantool_plot_mesh(cfg.mesh, 'edgecolor', 'none', 'vertexcolor', cfg.mesh.sulc);
          end
          hold on
          axis equal
          axis off
      ...
      otherwise
        artifactscantool_warning('invalid button (%d)', k);
    end
  end
  ...
  bst_artifactscantool_error('no layout detected, please specify cfg.layout')
end

% make the subset as specified in cfg.channel
cfg.channel = bst_ast_channelselection(cfg.channel, setdiff(layout.label, {'COMNT', 'SCALE'}));  % COMNT and SCALE are not really channels
chansel = bst_ast_match_str(layout.label, cat(1, cfg.channel(:), 'COMNT', 'SCALE'));                % include COMNT and SCALE, keep all channels in the order of the layout
...
if (~isfield(layout, 'outline') || ~isfield(layout, 'mask')) && ~strcmpi(cfg.style, '3d')
  % the reason to check for style=3d rather than 2d is that cfg.style is also an option in artifactscantool_topoplotER and artifactscantool_topoplotTFR
...
if ~strcmp(cfg.montage, 'no')
  ...
  new = artifactscantool_apply_montage(tmp, cfg.montage);
  layout.pos   = new.tra;
...
if ~skipcomnt && ~any(strcmp('COMNT', layout.label)) && ~strcmpi(cfg.style, '3d') && ~isequal(cfg.commentpos, 'title')
  ...
  elseif isnumeric(cfg.commentpos)
    ...
    bst_artifactscantool_error('invalid specification of cfg.commentpos');
  end
  layout.label{end+1}  = 'COMNT';
...
if ~skipscale && ~any(strcmp('SCALE', layout.label)) && ~strcmpi(cfg.style, '3d')
  ...
  elseif isnumeric(cfg.scalepos)
    ...
    bst_artifactscantool_error('invalid specification of cfg.scalepos');
  end
  layout.label{end+1}  = 'SCALE';
  layout.width(end+1)  = width;
  layout.height(end+1) = height;
...
if strcmp(cfg.feedback, 'yes') && ~strcmpi(cfg.style, '3d')
  ...
  artifactscantool_layoutplot(tmpcfg); % FIXME this should use artifactscantool_plot_layout
end

% to write the layout to a .mat or text file, you can use this code snippet
if ~isempty(cfg.output) && ~strcmpi(cfg.style, '3d')
  bst_artifactscantool_info('writing layout to ''%s''\n', cfg.output);
...
elseif ~isempty(cfg.output) && strcmpi(cfg.style, '3d')
  ...
  bst_artifactscantool_error('writing a 3D layout to an output file is not supported');
end

% do the general cleanup and bookkeeping at the end of the function
%artifactscantool_postamble provenance
% artifactscantool_postamble previous data
% artifactscantool_postamble history layout
% artifactscantool_postamble savevar layout
...
if ~exist(filename, 'file')
  bst_artifactscantool_error('could not open layout file "%s"', filename);
end
fid=fopen(filename);
...
elseif any(all(isnan(sens.chanpos)))
  [sel1, sel2] = bst_ast_match_str(sens.label, sens.labelold);
  sens.chanpos = chanposold(sel2, :);
  sens.label   = sens.labelold(sel2);
end

bst_artifactscantool_info('creating layout for %s system\n', bst_artifactscantool_senstype(sens));

% apply rotation, but only if viewpoint is not used specifically
if isempty(viewpoint)
  if isempty(rotatez)
    switch bst_artifactscantool_senstype(sens)
  ...
  sens.chanpos = bst_artifactscantool_warp_apply(rotate([0 0 rotatez]), sens.chanpos, 'homogenous');
end
...
if strcmpi(style, '3d')
  ...
  boxchannel = bst_ast_channelselection(boxchannel, label);
  boxchansel = bst_ast_match_str(label, boxchannel);
  ...
  if size(unique(prjForDist,'rows'),1) / size(prjForDist,1) < 0.8
    if strcmp(overlap, 'shift')
      artifactscantool_warning('the specified sensor configuration has many overlapping channels, creating a layout by shifting them around - use a template layout for better control over the positioning');
      prj = shiftxy(prj', 0.2)';
      prjForDist = prj(boxchansel,:);
    elseif strcmp(overlap, 'no')
      bst_artifactscantool_error('the specified sensor configuration has many overlapping channels, you specified not to allow that');
    elseif strcmp(overlap, 'keep')
      prjForDist = unique(prj(boxchansel,:), 'rows');
    else
      bst_artifactscantool_error('unknown value for cfg.overlap = ''%s''', overlap);
    end
  end
  ...
  if any(isfinite(d(:)))
    ...
    if any(strcmp(bst_artifactscantool_senstype(sens),{'eeg','unknown'})) && ~isempty(viewpoint) && (numel(boxchannel) ~= numel(label))
      mindist = min(d);
...
layout.pos = bst_artifactscantool_warp_apply(rotate([0 0 rotatez]), layout.pos, 'homogenous');
...
if size(pos,2)~=3
  bst_artifactscantool_error('XYZ coordinates are required to obtain the orthographic projections based on a viewpoint')
end
...
switch coordsys
  case {'ras' 'itab' 'neuromag' 'acpc' 'spm' 'mni' 'tal'}
    switch viewpoint
      ...
      otherwise
        bst_artifactscantool_error('orthographic projection using viewpoint "%s" is not supported', viewpoint)
  ...
  otherwise
    bst_artifactscantool_error('orthographic projection using coordinate system "%s" is not supported', coordsys)
end % switch coordsys

% extract xy
pos      = bst_artifactscantool_warp_apply(transmat, pos, 'homogenous');
pos      = pos(:,[1 2]);
...
if bst_artifactscantool_platform_supports('boundary')
  k = boundary(boxpos,.2);
  outline{1} = boxpos(k,:);
else
...
if ~isempty(cfg.headshape)
  if ischar(cfg.headshape) && exist(cfg.headshape, 'file')
    bst_artifactscantool_info('reading headshape from file %s\n', cfg.headshape);
    outlbase = artifactscantool_read_headshape(cfg.headshape);
  elseif isstruct(cfg.headshape)
    outlbase = cfg.headshape;
  else
    bst_artifactscantool_error('incorrect specification of cfg.headshape')
  end
elseif ~isempty(cfg.mri)
  if ischar(cfg.mri) && exist(cfg.mri, 'file')
    bst_artifactscantool_info('reading MRI from file %s\n', cfg.mri);
    outlbase = artifactscantool_read_mri(cfg.mri);
  elseif artifactscantool_datatype(cfg.mri, 'volume')
    outlbase = cfg.mri;
  else
    bst_artifactscantool_error('incorrect specification of cfg.mri')
  end
  ...
  outlbase = bst_artifactscantool_prepare_mesh(cfgpm, outlbase);
end
...
assert(isfield(outlbase, 'coordsys'), 'no coordsys field found in headshape/mri, use artifactscantool_determine_coordsys')
assert(isfield(sens, 'coordsys'), 'no coordsys field found in sensor structure, use artifactscantool_determine_coordsys')
...
outlbase = bst_artifactscantool_convert_units(outlbase, sens.unit);
...
for i=1:numel(outlbase)
  ...
  if bst_artifactscantool_platform_supports('boundary')
    ...
    if isequal(cfg.projection,'orthographic') && ~isempty(cfg.viewpoint)
      ...
      braincoords = bst_artifactscantool_warp_apply(rotate([0 0 rotatez]), braincoords, 'homogenous');
    ...
    if isequal(cfg.projection,'orthographic') && ~isempty(cfg.viewpoint)
      ...
      outlbase(i).pos = bst_artifactscantool_warp_apply(rotate([0 0 rotatez]), outlbase(i).pos, 'homogenous');
      outlbase(i).pos = elproj(outlbase(i).pos, cfg.projection);
    end
    h = figure('visible', 'off');
    bst_artifactscantool_plot_mesh(outlbase(i), 'facecolor', [0 0 0], 'EdgeColor', 'none');
    view([0 90])
    axis tight
    xlim = get(gca, 'xlim');
    ylim = get(gca, 'ylim');
