function [channel] = artifactscantool_channelselection(desired, datachannel, senstype)
...
if nargin<3 || isempty(senstype)
  senstype = bst_ast_senstype(datachannel);
end
...
elseif isstruct(datachannel) && isfield(datachannel, 'label')
  ...
  datachantype = artifactscantool_chantype(hdr);
else
  artifactscantool_error('please specify the data channels as a cell-array');
end

if ~ischar(desired) && ~isnumeric(desired) && ~iscell(desired)
  artifactscantool_error('please specify the desired channels as a cell-array or a string');
end
...
if length(datachannel)~=length(unique(datachannel))
  artifactscantool_warning('discarding non-unique channel names');
...
[dataindx, chanindx] = bst_ast_match_str(datachannel, channel);
...
switch senstype
  ...
  case {'ant128', 'biosemi64', 'biosemi128', 'biosemi256', 'egi32', 'egi64', 'egi128', 'egi256', 'eeg1020', 'eeg1010', 'eeg1005', 'ext1020'}
    if ~bst_ast_senstype(datachannel, 'unknown')
      % use an external helper function to define the list with EEG channel names
      labeleeg = artifactscantool_senslabel(bst_ast_senstype(datachannel));
    end
...
if ~isempty(badchannel)
  ...
  badchannel = artifactscantool_channelselection(badchannel, datachannel); % support exclusion of channel groups
end
...
badindx = bst_ast_match_str(channel, badchannel);
channel(badindx) = [];

% remove channel labels that are not present in the data
chanindx = bst_ast_match_str(channel, datachannel);
channel  = channel(chanindx);

if findgui
  indx = select_channel_list(datachannel, bst_ast_match_str(datachannel, channel), 'Select channels');
...
if isempty(channel) && ~recursion
  ...
  recursion = true;
  channel   = artifactscantool_channelselection(desired, lower(datachannel));
  recursion = false;
...
if isempty(channel) && ~recursion
  ...
  recursion = true;
  channel = artifactscantool_channelselection(desired, upper(datachannel));
  recursion = false;
...
% undo the sorting, make the order identical to that of the data channels
[tmp, indx] = bst_ast_match_str(datachannel, channel);
channel = channel(indx);
